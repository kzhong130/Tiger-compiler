...
bool ExistInList(Temp_temp t, Temp_tempList l)
{
    Temp_tempList p = l;
    for (; p; p = p->tail) {
        if (p->head == t) {
...
bool EqualList(Temp_tempList l, Temp_tempList r)
{
    Temp_tempList p = l;
    for (; p; p = p->tail) {
        if (!ExistInList(p->head, r)) {
            return FALSE;
        }
    }
    p = r;
    for (; p; p = p->tail) {
        if (!ExistInList(p->head, l)) {
            return FALSE;
        }
    }
    return TRUE;
...
Temp_tempList MinusList(Temp_tempList l, Temp_tempList r)
{
...
Temp_tempList UnionList(Temp_tempList l, Temp_tempList r)
{
    Temp_tempList res = r;
    Temp_tempList p = l;
    for (; p; p = p->tail) {
